...
// 1. ScalarValue value (string, int, bool, etc).
//		Ex.:
//			user1/networks/ip: "::/0"
// 2. VectorValue of scalars
//		Ex.:
//			user1/networks/ip:
//				- "127.0.0.1"
//				- "192.168.1.2"
...
// Setting represents one settings, which can be either a sting or a VectorValue of strings
type Setting struct {
	ScalerType  bool     `json:"-"`
	ScalarValue string   `json:"-"`
	VectorValue []string `json:"-"`
}

// NewScalarSetting makes new ScalarValue Setting
...
// NewVectorSetting makles new VectorValue Setting
...
// ScalerType checks whether setting is a ScalarValue value
func (s *Setting) IsScalar() bool {
	return s.ScalerType
}

// IsVector checks whether setting is a VectorValue value
func (s *Setting) IsVector() bool {
	return !s.ScalerType
}

// Scalar gets ScalarValue value of a setting
func (s *Setting) Scalar() string {
	return s.ScalarValue
}

// Vector gets VectorValue values of a setting
func (s *Setting) Vector() []string {
	return s.VectorValue
}

// AsVector gets value of a setting as VectorValue. Scalar value is casted to VectorValue
...
func (s *Setting) String() string {
	...
	return strings.Join(s.VectorValue, ",")
}
...
func (settings Settings) MarshalJSON() ([]byte, error) {
	...
	for name, setting := range settings {
		if setting.ScalerType {
			raw[name] = setting.ScalarValue
		} else {
			raw[name] = setting.VectorValue
		}
	}
