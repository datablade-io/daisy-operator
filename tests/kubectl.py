...
def delete_chi(chi, ns=namespace):
    with When(f"Delete daisyinstallation {chi}"):
        launch(f"delete daisyinstallation {chi}", ns=ns, timeout=900)
        wait_objects(
            chi,
            {
                "statefulset": 0,
...
def delete_all_chi(ns=namespace):
    ...
    if "daisyinstallations.daisy.com" in crds:
        chis = get("daisyinstallation", "", ns=ns)["items"]
        for chi in chis:
            # kubectl(f"patch daisyinstallation {chi} --type=merge -p '\{\"metadata\":\{\"finalizers\": [null]\}\}'", ns = ns)
...
def create_and_check(config, check, ns=namespace, timeout=30):
    ...
    if "pod_count" in check:
        wait_object("pod", "", label=f"-l daisy.com/chi={chi_name}", count=check["pod_count"], ns=ns)
...
def wait_objects(chi, object_counts, ns=namespace):
    with Then(
        ...
        for i in range(1, max_retries):
            cur_object_counts = count_objects(label=f"-l app.kubernetes.io/instance={chi}", ns=ns)
...
def wait_chi_status(chi, status, ns=namespace, retries=max_retries):
    wait_field("daisyinstallation", chi, ".status.status", status, ns, retries)
...
def get_pod_spec(chi_name, ns=namespace):
    pod = get("pod", "", ns=ns, label=f"-l daisy.com/chi={chi_name}")["items"][0]
    return pod["spec"]
...
def get_pod_names(chi_name, ns=namespace):
    pod_names = launch(
        f"get pods -o=custom-columns=name:.metadata.name -l daisy.com/chi={chi_name}",
        ns=ns,
    ).splitlines()
    return pod_names[1:]
        ...
        "requiredDuringSchedulingIgnoredDuringExecution": [
            {
                "labelSelector": {
                    "matchLabels": {
                        "daisy.com/app": "chop",
                        "daisy.com/chi": f"{chi_name}",
                        "daisy.com/namespace": f"{ns}",
                    },
                },
